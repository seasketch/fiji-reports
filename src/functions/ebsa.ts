...
/**
 * ebsa: A geoprocessing function that calculates overlap metrics for vector datasources
 * @param sketch - A sketch or collection of sketches
 * @param extraParams
 * @returns Calculated metrics and a null sketch
 */
export async function ebsa(
  sketch:
  ...
  const metricGroup = project.getMetricGroup("ebsa");
  const metrics = (
    await Promise.all(
...
export default new GeoprocessingHandler(ebsa, {
  title: "ebsa",
  description: "",
  timeout: 500, // seconds
